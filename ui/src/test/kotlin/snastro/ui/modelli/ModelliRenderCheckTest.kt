...
/**
 * `:ui:renderCheck` (profile `ui_render_check`): every [ModelliUiStato] fixture at both sizes —
 * sizing/overflow/contrast/state-rendering (AC-227..232). Six states: not downloaded (AC-227),
 * downloading with progress (AC-228), ready (AC-231/232), error — hash mismatch and error — no
 * network (AC-229/230), and partial (a Mancanti with fewer entries/bytes left than a fresh install,
 * distinguishing "nothing downloaded yet" from "some already installed"). [SchermataModelli] renders
...
class ModelliRenderCheckTest {
    ...
    private fun verificaMancanti(width: Int, height: Int) = runDesktopComposeUiTest(width, height) {
        setContent {
            SchermataModelli(ModelliUiStato.Mancanti(numero = 4, totaleByte = 521_000_000), AZIONI_VUOTE)
        }
        ...
        catturaPng("modelli-mancanti", width, height)
    }
    ...
    private fun verificaParziale(width: Int, height: Int) = runDesktopComposeUiTest(width, height) {
        setContent {
            SchermataModelli(ModelliUiStato.Mancanti(numero = 1, totaleByte = 26_530_550), AZIONI_VUOTE)
        }
        onNodeWithText(etichettaModelliMancanti(1)).assertIsDisplayed()
        onNodeWithText(formattaByte(26_530_550)).assertIsDisplayed()
        onNodeWithTag("modelli-scarica").assertIsDisplayed()
        catturaPng("modelli-parziale", width, height)
    }

    private fun verificaInDownload(width: Int, height: Int) = runDesktopComposeUiTest(width, height) {
        setContent {
            SchermataModelli(
        ...
        catturaPng("modelli-in-download", width, height)
    }

    private fun verificaErroreHash(width: Int, height: Int) = runDesktopComposeUiTest(width, height) {
        val messaggio = messaggioPer(ErroreServizioModelli.HashNonValido("asr-parakeet-tdt-0.6b-v3-int8"))
        setContent { SchermataModelli(ModelliUiStato.Errore(messaggio), AZIONI_VUOTE) }
        ...
        catturaPng("modelli-errore-hash", width, height)
    }

    private fun verificaErroreRete(width: Int, height: Int) = runDesktopComposeUiTest(width, height) {
        val messaggio = messaggioPer(ErroreServizioModelli.ReteAssente)
        setContent { SchermataModelli(ModelliUiStato.Errore(messaggio), AZIONI_VUOTE) }
        onNodeWithTag("modelli-errore").assertIsDisplayed()
        onNodeWithText(messaggio).assertIsDisplayed()
        onNodeWithTag("modelli-riprova").assertIsDisplayed()
        catturaPng("modelli-errore-rete", width, height)
    }

    private fun verificaPronti(width: Int, height: Int) = runDesktopComposeUiTest(width, height) {
        val licenze = listOf(
        ...
        setContent { SchermataModelli(ModelliUiStato.Pronti(licenze), AZIONI_VUOTE) }
        ...
        catturaPng("modelli-pronti", width, height)
    }
