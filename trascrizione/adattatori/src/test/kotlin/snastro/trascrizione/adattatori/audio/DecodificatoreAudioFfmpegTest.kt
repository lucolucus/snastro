...
import snastro.trascrizione.applicazione.porte.DecodificatoreAudioContratto
 ...
 * [DecodificatoreAudioFfmpeg] (AC-149). Exercising it opens real native FFmpeg for decode AND probe
 * ([DecodificatoreAudioFfmpeg.tutti] reads the derived WAV's duration back through
 * [snastro.audio.SondaFfmpeg]), so the whole class is `@Tag("modelli")` — opt-in, excluded from
 * `check`. No committed sample (the profile forbids one): [sorgente] is a synthetic silent WAV
 * written at test time ([scriviWavSintetico], the exact shape `:audio` itself writes and decodes in
 * its own `@Tag("modelli")` tests, `audio/src/test/kotlin/snastro/audio/DecodificaFfmpegTest.kt`) —
 * FFmpeg still does real work opening/resampling/re-muxing it, the same path a real source takes.
 */
@Tag("modelli")
class DecodificatoreAudioFfmpegTest : DecodificatoreAudioContratto() {
    ...
    // AC-150 below can find it again by id after the derived WAV is deleted.
    override val sorgente: RiferimentoAudio by lazy {
        val audio = cartellaProgetto.resolve("audio").also(Files::createDirectories)
        scriviWavSintetico(audio.resolve("${REGISTRAZIONE.valore}.wav"), durataMs = DURATA_MINIMA_MS)
        RiferimentoAudio("audio/${REGISTRAZIONE.valore}.wav")
    }

    override val sorgenteIlleggibile: RiferimentoAudio = RiferimentoAudio("audio/assente.wav")

    @Test
    fun `AC-150 un WAV derivato mancante viene ricostruito dalla sorgente copiata`() {
        ...
        assertTrue(Files.exists(wavDerivato), "il WAV derivato deve esistere dopo decodifica()")
        val primaDellaCancellazione = d.tutti(REGISTRAZIONE)

        Files.delete(wavDerivato)

        // Una nuova istanza: non ha memoria di decodifica() chiamato prima, eppure ricostruisce.
        val dopoLaRicostruzione = decodificatore().tutti(REGISTRAZIONE)

        assertTrue(Files.exists(wavDerivato), "il WAV derivato deve essere ricostruito dalla sorgente")
        assertEquals(primaDellaCancellazione, dopoLaRicostruzione)
    }
}
