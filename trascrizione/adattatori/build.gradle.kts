...
// Opt-in real-native contract (ADR 0005/0016): the whole `DecodificatoreAudioFfmpegTest` and
// `DiarizzatoreSherpaTest` classes are `@Tag("modelli")` (every contract case runs a real native
// first), so the default `test` task (which excludes "modelli", dev-architecture-app.md#test) runs
// none of them. A dedicated `Test` task configures its OWN `useJUnitPlatform` and does not inherit
// that exclusion (the comment on `configureTesting()` in build-logic). Never wired into `check`.
// `scaricaNativiSherpa` + `sherpa_onnx.native.path` mirror `:ml-sherpa`'s own `modelliTest`
// (AC-243/244, ADR 0016 §4) — `DiarizzatoreSherpaTest` loads the real sherpa-onnx natives too.
val scaricaNativiSherpa = rootProject.tasks.named("scaricaNativiSherpa")

tasks.register<Test>("modelliTest") {
    group = "verification"
    description = "Opt-in: DecodificatoreAudioFfmpegTest (real FFmpeg, AC-149/AC-150) + " +
        "DiarizzatoreSherpaTest (real sherpa-onnx, AC-249/AC-373)."
...
dependencies {
    ...
    // DiarizzatoreSherpa delegates to :ml-sherpa's MotoreSherpa/SessioneSherpa (native load, Mutex,
    // resource registration), never touching com.k2fsa directly (ADR 0004, CR-3 confinement).
