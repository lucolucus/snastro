...
- **Migrations are forward-only**: numbered `.sqm` files in `persistenza/src/main/sqldelight/
  ...
  migrations. The `:persistenza:test` migration test (Amendment (a)) verifies this in
  `./gradlew check`. Schema version is applied at open; a DB newer than the app refuses to open
  with a clear message.
- ~~**Scoped exception to the profile rule "never commit DB files"**~~ (superseded by Amendment (a):
  the committed schema snapshots this once needed are gone, so the exception no longer applies).
