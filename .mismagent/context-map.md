...
- **Ubiquitous language:**
  ...
  - `EstrattoAudio` = the ~10 s of audio of a `Voce` the user listens to before deciding: its 2–3 longest `Segmento`s concatenated (user decision 2026-09-23). For a `Candidato`, built the same way from the `Voce` that sourced one of its `ImprontaVocale`s. Not: snippet, anteprima, campione.
