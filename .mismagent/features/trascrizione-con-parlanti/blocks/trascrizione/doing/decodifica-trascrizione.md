...
related_adrs:
  ...
  - "0012"
---
# decodifica-trascrizione — DecodificatoreAudio (trascrizione) su :audio

## What to do
Thin adapter implementing DecodificatoreAudio (trascrizione) over the :audio API, resolving project-relative paths (audio/<id>.<ext>, cache/audio/<id>.wav — rebuilt from the source if missing).

## Tasks
- AC-149 [@modelli] DecodificatoreAudioContratto (trascrizione) passano contro l'adattatore reale su audio sintetico scritto dal test (un WAV generato a runtime; nessun file di sample/ nei test)
- AC-150 Un WAV derivato mancante viene ricostruito dalla sorgente copiata
