...
view_sources:
  RegistrazioneVista: "≡ pinned type of registrazione-per-trascrizione / registrazione-per-parlanti (rule 16) ← Registrazione aggregate via RegistrazioneRepository"
---
# catalogo-registrazioni — CatalogoRegistrazioni (API pubblica del Progetto)

## What to do
Supplier query API registrazione(id): RegistrazioneVista? — its shape IS the pinned type.

### view_shape (field ← source)
- `RegistrazioneVista`: registrazioneId, progettoId, titolo, riferimentoAudio, dataRegistrazione, durataMs ← ≡ pinned type of registrazione-per-trascrizione / registrazione-per-parlanti (rule 16) ← Registrazione aggregate via RegistrazioneRepository

## Tasks
- AC-97 registrazione(id) restituisce registrazioneId, progettoId, titolo, riferimentoAudio, dataRegistrazione e durataMs della Registrazione; id sconosciuto → null
