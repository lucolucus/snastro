...
## Diarization rework experiment (2026-09-24, full Via Roquel, ≤6 threads) — no transcript text
Finding: the app reproduces the spike exactly; the instability was the pipeline (ResNet34-LM embeddings + sherpa FastClustering with numClusters, no minimum cluster size), not the app nor (mainly) the recording. 10 ms audio shift changed the old setup's split from [1874,737,367,349] to [2094,756,470,12].
| setup | s/speaker | stability mean (min) over +10/+48/+500 ms shifts | wall 75 min |
|---|---|---|---|
| current: sherpa Fast k4 + ResNet34-LM | 1874/737/367/349 | 0.63 (0.54) | 89 s |
| sherpa Fast k4 + TitaNet-S / -L / ERes2Net | outlier slot (2–24 s) each | 0.55–0.78 | 45–75 s |
| own AHC k4 (min cluster 60 s) + TitaNet-S, seg fp32 | 1136/971/803/584 | **0.940 (0.928)** | ~100 s |
| own AHC k4 + TitaNet-L, seg fp32 | 1154/946/815/579 | 0.940 (0.928) | ~125 s |
